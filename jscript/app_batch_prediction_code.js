...
async function model_makePrediction(fname) {
  ...
  top5.forEach(function (p) {
    $(
      "#prediction-list"
    ).append(`<li style="list-style-type:none;">${p.className}: ${p.probability.toFixed(3)}<p><a style="color: blue; text-decoration: underline;" href="${randomPropertyValue}"> ${p.className} Consultant Link</a><p>
		<p>${detailRemedy(p.className)}</p></li>`);
    sendDataToPHP(p.className, p.probability.toFixed(3));
  });
...
// AJAX function to send data to PHP
